class StoreRegistrationsController < Devise::RegistrationsController
  ...
  if Rails.env.production?
    if request.referer == 'https://www.kduka.co.ke/explore' || request.referer == 'https://www.kduka.co.ke/stores/login' || request.referer == 'https://www.kduka.co.ke/'
    else
...
=end
  ...
  def create
    ...
    @store = Store.create(store_params.merge(display_email: params[:store][:email], subdomain: santize(params[:store][:subdomain]), layout_id: 1, store_color: '#7aae42', homepage_status: true, aboutpage_status: true, manual_delivery_status: false, auto_delivery_status: false, collection_point_status: false, init: false, important: false, active: false, trial:true, trial_start:trial_start,trial_end:trial_end, activatable:true))
